...
import { Brain, Calendar, Mail, Loader2, CheckCircle, AlertCircle, Download } from "lucide-react";
...
const AIRecruitmentPanel = ({ jobId, applications = [], onRefresh }: AIRecruitmentPanelProps) => {
  ...
  const handleDownloadReport = () => {
    const analyzedApps = applications.filter(app => app.ai_match_score);
    
    const reportContent = `
AI RECRUITMENT ANALYSIS REPORT
================================
Generated: ${new Date().toLocaleString()}
Job ID: ${jobId || 'All Jobs'}

SUMMARY
-------
Total Applications Analyzed: ${analyzedApps.length}
Average Match Score: ${analyzedApps.reduce((sum, app) => sum + (app.ai_match_score || 0), 0) / analyzedApps.length || 0}/100
...
${index + 1}. ${app.candidate_name}
   ...
   Match Score: ${app.ai_match_score}/100
   Status: ${app.status}
   
   AI Analysis:
   ${app.ai_analysis || 'No analysis available'}
...
RECOMMENDATIONS
---------------
Strong Recommendations (80+): ${analyzedApps.filter(a => (a.ai_match_score || 0) >= 80).length} candidates
Good Matches (60-79): ${analyzedApps.filter(a => (a.ai_match_score || 0) >= 60 && (a.ai_match_score || 0) < 80).length} candidates
Consider (Below 60): ${analyzedApps.filter(a => (a.ai_match_score || 0) < 60).length} candidates

This report was generated by AI analysis and should be used as a supporting tool for recruitment decisions.
All final hiring decisions should be made by qualified HR personnel.
`;
    ...
    a.href = url;
    a.download = `AI_Recruitment_Report_${new Date().toISOString().split('T')[0]}.txt`;
    ...
    toast({
      ...
      description: "AI recommendations saved successfully",
    });
  };
